 ...
 */
const arrayType = (array) => mobx.toJS(array);
 ...
 */
const objectType = (object) => mobx.toJS(object);
 ...
 */
function diff (a = {}, b = {}) {
  return flattenDeep__default["default"](walk(a, (prevValue, path) => {
    const currValue = dig__default["default"](b, path);
...
function parseErrorPointer (error = {}) {
  ...
  const match = dig__default["default"](error, 'source.pointer', '').match(regex);
...
const QueryString = {
   ...
   */
  parse: (str) => qs__default["default"].parse(str, { ignoreQueryPrefix: true }),
  /**
   ...
   */
  stringify: (params) => qs__default["default"].stringify(params, { arrayFormat: 'brackets' })
};
...
class Store {
  ...
  add (type, props = {}, options) {
    if (props.constructor.name === 'Array') {
      ...
      const attributes = cloneDeep__default["default"](this.pickAttributes(props, type));
  ...
  pickRelationships (properties, type) {
    ...
    return definitions ? pick__default["default"](properties, Object.keys(definitions)) : {}
  }
  ...
  fetchMany (type, ids, options = {}) {
    ...
    const queries = idQueries.map((queryIds) => {
      const params = cloneDeep__default["default"](queryParams);
  ...
  */
  ...
  getAll (type, options = {}) {
    ...
    if (queryParams) {
      ...
      return uniqBy__default["default"](this.getRecords(type).filter((record) => record.initialized), 'id')
    }
  }
  ...
  getCachedIds (type, url) {
    ...
    if (!ids) return []
    const idsSet = new Set(mobx.toJS(ids));
 ...
 */
const definitionsByDirection = mobx.action((model, direction) => {
 ...
 */
const defineToOneRelationships = mobx.action((record, store, toOneDefinitions) => {
 ...
 * @returns {object} an object with getters and setters based on the defintions
 */
const defineToManyRelationships = mobx.action((record, store, toManyDefinitions) => {
 ...
 */
const setRelatedRecord = mobx.action((relationshipName, record, relatedRecord, store, inverse) => {
 ...
 */
const removeRelatedRecord = mobx.action((relationshipName, record, relatedRecord, inverse) => {
 ...
 */
const addRelatedRecord = mobx.action((relationshipName, record, relatedRecord, inverse) => {
 ...
 */
const coerceDataToExistingRecord = mobx.action((store, record) => {
...
class Model {
  ...
  get dirtyAttributes () {
    ...
    return Object.keys(this.attributes).reduce((dirtyAccumulator, attr) => {
      ...
      if (isObject__default["default"](currentValue)) {
        ...
        union__default["default"](currentToPreviousDiff, previousToCurrentDiff).forEach((property) => {
  ...
  get persistedOrFirstSnapshot () {
    return findLast__default["default"](this._snapshots, (ss) => ss.persisted) || this._snapshots[0]
  }
  ...
  takeSnapshot (options = {}) {
    ...
    const properties = cloneDeep__default["default"](pick__default["default"](this, ['attributes', 'relationships']));
  ...
  get attributes () {
    return this.attributeNames.reduce((attributes, key) => {
      const value = mobx.toJS(this[key]);
      if (value != null) {
  ...
  jsonapi (options = {}) {
    ...
    if (options.relationships) {
      ...
      const relationships = filteredRelationshipNames.reduce((rels, key) => {
        rels[key] = mobx.toJS(this.relationships[key]);
  ...
  updateAttributes (attributes) {
    ...
    const validAttributes = pick__default["default"](attributes, attributeNames);
...
class FactoryFarm {
  ...
  build (factoryName, overrideOptions = {}, numberOfRecords = 1) {
    ...
    if (numberOfRecords > 1) {
      addProperties = times__default["default"](numberOfRecords, (i) => _buildModel(factoryName, newModelProperties, i));
    } else {
...
const addIncluded = (store, encodedModel, included, allEncoded = [encodedModel]) => {
    ...
    Object.keys(relationships).forEach((key) => {
        ...
        const notAlreadyIncluded = data.filter(({ id, type }) => !allEncoded.some((encodedModel) => encodedModel.type === type && encodedModel.id === id));
 ...
 */
const serverResponse = function (modelOrArray, options = {}) {
    var _a;
    let model;
    let array;
    let encodedData;
    ...
    return JSON.stringify({
        ...encodedData,
        version: (_a = options.version) !== null && _a !== void 0 ? _a : '1.0'
    });
};
/**
...
class MockServer {
    ...
    constructor(options = {}) {
        ...
        this.jsonapiVersion = (_a = options.jsonapiVersion) !== null && _a !== void 0 ? _a : '1.0';
    }
    /**
    ...
    start(options = {}) {
        ...
        fetch.mockResponse((req) => {
            ...
            return wrapResponse({ response, method: req.method, status: foundQuery === null || foundQuery === void 0 ? void 0 : foundQuery.status });
        });
    }
    /**
