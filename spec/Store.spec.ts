...
import { computed, isObservable, makeObservable, toJS } from 'mobx'
...
interface IAppStore extends IStore {
  loadingTodos: boolean
}

class AppStore extends Store implements IAppStore {
  constructor() {
    super()
    makeObservable(this, {
      loadingTodos: computed
    })
  }

  get loadingTodos (): boolean {
...
const createMockIds = (numberOfIds: number, idPrefix = '') => {
...
describe('Store', () => {
  ...
  //   expect(store._baseUrl).toEqual(mockBaseUrl)
  //   expect(store._defaultFetchOptions).toEqual(mockFetchOptions)
  //   expect(store._headersOfInterest).toEqual(['X-Mobx-Example'])
  // })
  ...
  // // Could not get `fetchMock.mockResponse` to mock headers, so had to comment out these tests
  // describe('lastResponseHeaders', () => {
  //   it('captures interesting headers from http responses', async () => {
  //     fetchMock.mockResponse({ body: 'data: {}', headers: { 'X-Ignore-Me': 'Ignore', 'X-Mobx-Example': '123' } })
  //     store.fetchOne('todos', 1)

  //     expect(store.lastResponseHeaders).toEqual({'X-Mobx-Example': '123'})

  //     fetchMock.mockResponse({ body: 'data: {}', headers: { 'X-Ignore-Me': 'Ignore', 'X-Mobx-Example': 'ABC' } })
  ...
  describe('bulkSave', () => {
    ...
    it('constructs a payload for all records in a jsonapi bulk-extension compliant way', async () => {
      ...
      fetchMock.mockResponse(mockTodosResponse)
  ...
  describe('bulkCreate', () => {
    ...
    beforeEach(() => {
      const backendStore: IStore = new AppStore()
  ...
  describe('updateRecordsFromResponse', () => {
    function mockRequest (errors, status: number = 422) {
  ...
  describe('fetchOne', () => {
    ...
    it('identifies relationships, even when not returned from server', async () => {
      fetchMock.mockResponseOnce(mockTodoWithMetaDataResponse)
    ...
    it('keeps relationships on successive fetches', async () => {
      fetchMock.mockResponseOnce(mockTodoWithNotesResponse)
      fetchMock.mockResponseOnce(mockTodoWithTagsResponse)
    ...
    it('supports queryParams', async () => {
      ...
      expect(decodeURIComponent(fetchMock.mock.calls[0][0])).toEqual(
    ...
    describe('error handling', () => {
      ...
      it('uses the configured error message for the corresponding HTTP status code', async () => {
        ...
        fetchMock.mockResponses(['', { status: 403 }])
  ...
  describe('findAll', () => {
    ...
    it('returns cached meta', async () => {
      fetchMock.mockResponse(mockTodosResponseWithMeta)
      // Populate the cache
      await store.findAll('todos', {
        queryParams: {
          title: 'Do taxes'
  ...
  describe('fetchAll', () => {
    it('always fetches the records with the given type from the server', async () => {
      fetchMock.mockResponse(mockAllTodosResponse)
    ...
    it('only fetches once when called multiple times', async () => {
      ...
      fetchMock.mockResponseOnce(() => (new Promise((resolve) => setTimeout(() => resolve(mockResponseData), 100))))
      fetchMock.mockResponseOnce(Promise.resolve(mockResponseData))
  ...
  describe('fetchMany', () => {
    it('returns a promise with the records of the given type and id', async () => {
      expect.assertions(5)
      fetchMock.mockResponse(createMockTodosResponse(5, '1000'))
    ...
    it('uses multiple fetches for data from server', async () => {
      expect.assertions(7)
      fetchMock.mockResponseOnce(createMockTodosResponse(100, '1000'))
      fetchMock.mockResponseOnce(createMockTodosResponse(100, '1100'))
      fetchMock.mockResponseOnce(createMockTodosResponse(100, '1200'))
  ...
  describe('findMany', () => {
    ...
    describe('some records of the specified type and ids are the store', () => {
      it('uses multiple fetches to request the rest of the records from the server', async () => {
        ...
        fetchMock.mockResponseOnce(createMockTodosResponse(100, '1000'))
        fetchMock.mockResponseOnce(createMockTodosResponse(75, '1100'))
        ...
        expect(
          fetchMock.mock.calls.some((call) => call[0].match(/1173/))
        ).toBeTruthy()
        expect(
          fetchMock.mock.calls.some((call) => call[0].match(/1174/))
        ).toBeTruthy()
        expect(
          fetchMock.mock.calls.some((call) => call[0].match(/1175/))
        ).toBeFalsy()
    ...
    it('fetches data with other params', async () => {
      ...
      const [[firstPath], [secondPath], [thirdPath]] = fetchMock.mock.calls
    ...
    it('fetches data with named array filters', async () => {
      ...
      const [firstPath] = fetchMock.mock.calls[0]
