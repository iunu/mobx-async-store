...
class Store {
  ...
  */
  async findMany (type, ids, options = {}) {
    ...
    const { persistedIds } = this.data[type]
    idsToQuery = idsToQuery.filter((id) => !persistedIds.has(id))
  ...
  async fetchAll (type, options = {}) {
    ...
    if (response.status === 200) {
      ...
      runInAction(() => {
        ...
        this.data[type].persistedIds = new Set([...recordIds, ...this.data[type].persistedIds])
