...
class Store {
  ...
  addModel = (type, attributes) => {
    ...
    const model = this._createModel(type, id, { attributes })
  ...
  findOrFetchOne = (type, id, queryParams) => {
    ...
    const record = this._getMatchingRecord(type, id, queryParams)
  ...
  findAll = (type, options = {}) => {
    ...
    if (fromServer === true) {
      ...
      // If fromServer is false never fetch the data and return
      return this._getMatchingRecords(type, queryParams)
    } else {
      return this._findOrFetchAll(type, queryParams)
    }
  }
  ...
  async fetchOne (type, id, queryParams) {
    ...
    if (response.status === 200) {
      ...
      const record = this._createOrUpdateOneRecordFromResponseData(data)
  ...
  getRecord (type, id) {
    ...
    const record = this._getType(type).records[id]
   ...
   */
  getRecords (type) {
    const records = Object.values(this._getType(type).records)
   ...
   */
  _initializeNetworkConfiguration (options = {}) {
   ...
   */
  _initializeObservableDataProperty () {
    const { types } = this.constructor
   ...
   */
  _getMatchingRecord (type, id, queryParams) {
    if (queryParams) {
   ...
   * @return {Array} array or records
   */
  _getCachedRecord (type, id, queryParams) {
   ...
   * @return {Array} array or records
   */
  _getCachedRecords (type, queryParams, id) {
    // Get the url the request would use
    const url = this.fetchUrl(type, queryParams, id)
    // Get the matching ids from the response
    const ids = this._getCachedIds(type, url)
   ...
   */
  _getCachedIds (type, url) {
    const ids = this._getType(type).cache[url]
    if (!ids) return []
   ...
   * @return {Array} array or records
   */
  _getMatchingRecords (type, queryParams) {
    if (queryParams) {
   ...
   * @param {Object} queryParams will inform whether to return cached or fetch
   */
  _findOrFetchAll = (type, queryParams) => {
    // Get any matching records
    const records = this._getMatchingRecords(type, queryParams)
   ...
   */
  _createOrUpdateOneRecordFromResponseData (dataObject) {
    ...
    if (record) {
      ...
      record = this._createModel(type, id, { attributes, relationships })
   ...
   * @param {Array} data
   */
  _createOrUpdateAllRecordsFromResponseData (data) {
    return transaction(() => data.map(dataObject => {
      ...
      if (this._getType(dataObject.type)) {
        return this._createOrUpdateOneRecordFromResponseData(dataObject)
      }
    }))
  }
   ...
   */
  _createModel (type, id, data) {
    const { attributes = {}, relationships = {} } = toJS(data)
    const store = this
    const ModelKlass = this._getKlass(type)
