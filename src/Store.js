...
class Store {
  ...
  findOne = (type, id, options = {}) => {
    ...
    if ([true, false].includes(fromServer)) {
      console.warn(
        'DeprecationWarning: using `fromServer` options will be deprecated - use getOne, fetchOne, or findOne.'
      )
    }
    ...
    if (fromServer === true) {
      ...
      return Promise.resolve(this.getOne(type, id, options))
    }
  ...
  getMany = (type, ids) => {
    ...
    return this.getRecords(type).filter((record) =>
  ...
  fetchMany = (type, ids, options = {}) => {
    ...
    const [idQueries] = deriveIdQueryStrings(ids, baseUrl).map(String)
    queryParams.filter.ids = idQueries
  ...
  findMany = (type, ids, options = {}) => {
    ...
    if ([true, false].includes(fromServer)) {
      console.warn(
        'DeprecationWarning: using `fromServer` options will be deprecated - use getMany, fetchMany, or findMany.'
      )
    }
  ...
  findAll = (type, options = {}) => {
    ...
    if ([true, false].includes(fromServer)) {
      console.warn(
        'DeprecationWarning: using `fromServer` options will be deprecated - use getAll, fetchAll or findAll.'
      )
    }
