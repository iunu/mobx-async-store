...
function getRelatedRecords(record, property, modelType) {
    ...
    if (references && references.data) {
        ...
        relatedRecords = record.store
            ...
            .filter(function (rel) { return String(rel[foreignId_1]) === String(record.id); });
    }
...
function getRelatedRecord(record, property, modelType) {
    ...
    if (!reference || !reference.data)
        return;
    var _a = relationships[relationType].data, id = _a.id, type = _a.type;
    ...
    function RelatedRecordsArray(array, record, property) {
        ...
        _this.remove = function (relatedRecord) {
            ...
            if (relationships && relationships[property] && relatedRecord) {
                var referenceIndexToRemove = relationships[property].data.findIndex(function (model) { return model.id === id && model.type === type; });
                relationships[property].data.splice(referenceIndexToRemove, 1);
                var recordIndexToRemove = _this.findIndex(function (model) { return model.id === id && model.type === type; });
